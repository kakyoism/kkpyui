...
def _test_form():
    ...
    form = Form(Globals.root)
    form.layout()
    ctrlr = FormController(form)
    menu = FormMenu(Globals.root, ctrlr)
    menu.init(Globals.root)
    # Creating groups
    ...
    form.layout()
    action_bar = FormActionBar(Globals.root, ctrlr)
    action_bar.layout()
    progress_bar = ProgressBar(Globals.root, Globals.progressQueue)
    progress_bar.layout()
    progress_bar.poll()
...
def _test_controller():
    class OscillatorController(FormController):
        ...
        def submit(self, event=None):
            ...
            Globals.progressQueue.put(('/start', 0, 'Playing ...'))
            self.playing = True
            return True

        def cancel(self, event=None):
            self.sender.send_message('/play', 0)
            Globals.progressQueue.put(('/stop', 100, 'Stopped'))
            time.sleep(1)
    ...
    form.init([page])
    form.layout()
    action_bar = OnOffActionBar(Globals.root, ctrlr)
    action_bar.layout()
    wait_bar = WaitBar(Globals.root, Globals.progressQueue)
    wait_bar.layout()
    wait_bar.poll()
    Globals.root.mainloop()
